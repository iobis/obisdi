...
Then, select the __Marine Biodiversity data ingestion - OBIS__ format
...
In case you supply a `zip` file it should be composed of _only_ `csv` files. You can also convert the files with more than 50MB into lightweight `parquet` files by checking the box "Convert to parquet". This conversion is strongly advised as GitHub does not allow big files to be uploaded (the limit is 100MB).
